  ...
  */
  if (processLockfiles && repoDoc && repoDoc.files) {
    ...
    for (const commit of dedupedCommits) {
      ...
      console.info('starting lockfile update', {lockfilePath})
      ...
      console.info('received existing lockfile from GitHub')

      const isNpm = lockfilePath.includes('package-lock.json')
      try {
        console.log('### lockfilePath', {lockfilePath})
        console.log('### commitMessageTemplates', {commitMessageTemplates})
        console.log('### commit.content', {content: commit.content})
        console.log('### commit.content', {content: typeof commit.content})
        console.log('### oldLockfileContent', {oldLockfileContent})
        ...
        if (ok) {
          ...
          console.info('new lockfile contents produced')
          ...
          console.log('### lockFileCommitMessage', {lockFileCommitMessage})
  ...
  try {
    console.log('### owner, repoName, sha, newBranch', {owner, repoName, sha, newBranch})
