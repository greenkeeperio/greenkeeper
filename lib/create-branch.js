  ...
  if (processLockfiles && repoDoc && repoDoc.files) {
    ...
    for (const commit of dedupedCommits) {
      ...
      try {
        ...
        */
        let repositoryTokens = null
        try {
          repositoryTokens = await tokens.get(installationId)

          const datetime = new Date().toISOString().substr(0, 19).replace('T', '_')
          // write audit log entry to 'tokens-audit' db
          // log entry type: 'read'
          await tokens_audit.put({
            _id: `${installationId}:${repoDoc._id}:${datetime}:read`,
            keys: Object.keys(repositoryTokens.tokens[repoDoc._id])
          })
        } catch (e) {}
        const { ok, contents } = await getNewLockfile({ packageJson: commit.content, lock: oldLockfileContent, isNpm, repositoryTokens })
        if (ok) {
