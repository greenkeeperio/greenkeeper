  ...
  */
  if (processLockfiles && repoDoc && repoDoc.files) {
    for (const commit of commits) {
      ...
      // return new lockfile, or nothing if ok: false (this means the old and new lockfile are the same)
      const isNpm = lockfilePath.includes('package-lock.json')
      try {
        // getNewLockfile(updated package.json, old package-lock.json, isNpm)
