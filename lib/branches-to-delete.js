...
const { getMonorepoGroupNameForPackage } = require('./monorepo')
...
async function getDependencyBranchesToDelete ({changes, repositories, repositoryId, config}) {
  const dependencyChanges = getDependencyChanges(changes)  // map over dependencyChanges and find monorepos
  const monorepoDependencyChanges = _.flatten(dependencyChanges.map(change => {
    const monorepoGroupName = getMonorepoGroupNameForPackage(change.dependency)
