...
async function updateMonorepoReleaseInfo (dependency, distTags, distTag, versions) {
  ...
  npm.upsert(`monorepo:${monorepoGroupname}:${version}`, (oldDoc) => {
    return { distTags, distTag, dependency, versions, ...updatedAt(oldDoc) }
  })
}
...
async function deleteMonorepoReleaseInfo (dependency, version) {
  const { npm } = await dbs()
  try {
    npm.remove(`monorepo:${dependency}:${version}`)
  } catch (e) {
...
async function pendingMonorepoReleases () {
  ...
  return _.map(npm.query('monorepo-releases-by-time', {
    startkey: thirtyMinutesAgo,
    include_docs: true
  }, row => row.doc)) || []
}
