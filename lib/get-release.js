...
module.exports = async function ({ installationId, owner, repo, version, sha }) {
  ...
  try {
    ...
    try {
      ...
      try {
        const { tag } = await ghqueue.read(github => github.gitdata.getTag({ owner, repo, tag_sha: sha }))
  ...
  const gitHubMentionHoverCardRegex = /class="user-mention" data-hovercard-user-id="\d+" data-octo-click="hovercard-link-click" data-octo-dimensions="link_type:self"/gmi
  ...
  ).replace(gitHubMentionHoverCardRegex, '') // remove GitHub Mentions
