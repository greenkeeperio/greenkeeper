...
async function hasPaidAccount (accountId, log) {
  if (!accountId) return false

  const { payments } = await dbs()
  try {
    const doc = await payments.get(String(accountId))
    if (doc.plan === 'opensource' || doc.plan === 'free') return false
  } catch (error) {
    log.warn(`Could not get payment info for accountId ${accountId}`)
    if (error.status !== 404) return false
  }
  return true
}

async function maybeUpdatePaymentsJob ({ accountId, isPrivate, repositoryId }) {
  ...
  // this is for users without stripe account, who made their public repo private
  if (isPrivate && repositoryId && (!await hasStripeBilling(accountId))) {
    // open an issue: we need payment information
    return {
      data: {
