...
module.exports = async function worker (scheduleJob, channel, job) {
  ...
  try {
    ...
    const error = typeof err === 'object' && !(err instanceof Error) && err.message
  ...
  try {
    ...
    const isPaid = await hasPaidAccount(data.accountId, log)
    ...
    await Promise.mapSeries(newJobs, (
      ...
      const priority = isPaid ? 3 : (data.name === 'added' || data.name === 'create-initial-branch') ? 2 : 1
      return scheduleJob(
