...
function write (installationId, gen) {
  ...
  statsd.increment('queues.github.write.tagged', [installationId, getGitHubMethod(gen)])
  const stats = {
    type: 'write',
    queued: Date.now()
  }
...
function read (installationId, gen) {
  ...
  statsd.increment('queues.github.read.tagged', [installationId, getGitHubMethod(gen)])
  const stats = {
    type: 'read',
    queued: Date.now()
  }
