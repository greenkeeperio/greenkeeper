...
function write (installationId, gen) {
  ...
  statsd.gauge('queues.github_write_requests_tagged', [installationId, getGitHubMethod(gen)])
  const stats = {
    type: 'write',
    queued: Date.now()
  }
...
function read (installationId, gen) {
  ...
  statsd.gauge('queues.github_read_requests_tagged', [installationId, getGitHubMethod(gen)])
  const stats = {
    type: 'read',
    queued: Date.now()
  }
